...
let bigrams text =
  ...
  let rec aux i acc =
    if i >= len then List.rev acc
      ...
      let sym = String.sub text i 1 in aux (i + 1) ((sym ^ sym) :: acc)
...
let encrypt text =
  Random.init 42;
  let square1, square2 = random_square (), random_square () in
  let bigrams_list = bigrams text in
  let encrypted_bigrams = List.map (fun bg ->
    encrypt_bigram (bg.[0], bg.[1]) (square1, square2)) bigrams_list in
  String.concat "" (List.map (fun (a, b) -> String.make 1 a ^ String.make 1 b) encrypted_bigrams)
...
let decrypt text =
  ...
  let square1, square2 = random_square (), random_square () in
  let bigrams_list = bigrams text in
  let decrypted_bigrams = List.map (fun bg ->
    decrypt_bigram (bg.[0], bg.[1]) (square1, square2)) bigrams_list in
  String.concat "" (List.map (fun (a, b) -> String.make 1 a ^ String.make 1 b) decrypted_bigrams)
