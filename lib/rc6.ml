...
let encrypt plainbytes iv =
  ...
  for i = 0 to num_blocks - 1 do
    ...
    for j = 0 to block_length - 1 do
      if block_start + j < Bytes.length plainbytes
      then
        Bytes.set
          ciphertext
          (block_start + j)
          (char_of_int
             (int_of_char (Bytes.get plainbytes (block_start + j))
              lxor int_of_char (Bytes.get encrypted_iv j)))
    done;
    for j = 0 to block_length - 1 do
      if block_start + j < Bytes.length ciphertext
      then Bytes.set iv (block_start + j) (Bytes.get ciphertext (block_start + j))
    done
  done;
  ciphertext
;;
...
let decrypt cipherbytes iv =
  ...
  for i = 0 to num_blocks - 1 do
    ...
    let encrypted_iv = rc6_encrypt iv in
    for j = 0 to block_length - 1 do
      if block_start + j < Bytes.length cipherbytes
      then
        Bytes.set
          plaintext
          (block_start + j)
          (char_of_int
             (int_of_char (Bytes.get cipherbytes (block_start + j))
              lxor int_of_char (Bytes.get encrypted_iv j)))
    done;
    for j = 0 to block_length - 1 do
      if block_start + j < Bytes.length cipherbytes
      then Bytes.set iv (block_start + j) (Bytes.get cipherbytes (block_start + j))
    done
  done;
  plaintext
;;
